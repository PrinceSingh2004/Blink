...
const PAGE_MAP = {
    ...
    '/chat.html':         '/pages/messages.html',   // legacy alias
    '/explore.html':      '/pages/index.html',       // legacy alias
};
...
io.on('connection', (socket) => {
    ...
    socket.on('join_live', async ({ streamId, userId, username, role }) => {
        ...
        try {
            ...
            // Sync count back to stream record
            ...
            // If viewer, notify existing participants so broadcaster can find them or vice versa
            // Actually, we'll let the broadcaster handle the user_joined event.
    ...
    socket.on('signal', (data) => {
        // data contains { to: socketId, from: socketId, signal: {type, sdp/candidate} }
    ...
    socket.on('request_stream', (data) => {
        // data: { streamId, to: broadcasterSocketId }
